...
import { editSkill, fetchSkill } from "../../../../features/skill/skillSlice";
...
const EditSkill = ({ onClose, skill }: Props) => {
  ...
  const [image, setImage] = useState<File | null>(skill.icon || "");
  ...
  const handleSave = async() => {
    if (!skill?._id) {
      alert("No skill selected to edit");
      return;
    }
    const payload = {
      name,
   ...
   await dispatch(editSkill({id:skill._id,skillData:payload}))
   await dispatch(fetchSkill())
    onClose()
  };
